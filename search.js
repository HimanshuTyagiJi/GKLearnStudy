...
function searchTitles(event) {
    ...
    if (query.length > 0) {
        ...
        const uniqueResults = [...new Set(result.map(item => item.item.title))]; // Remove duplicates
...
function showSuggestions(event) {
    ...
    if (query.length > 0) {
        ...
        const uniqueSuggestions = [...new Set(suggestions.map(item => item.item.title))]; // Remove duplicates
        uniqueSuggestions.forEach(({ item }) => {
            const suggestionItem = document.createElement('li');
            suggestionItem.innerHTML = item.title; // Show title without highlight
