...
document.addEventListener("DOMContentLoaded", function() {
  // --- 📢 Manual AdSense Ads ---
  const manualAds = [
    { id: "top-ad", slot: "9133141216", format: "auto" },        // Top GK_Article_Top_Square
    { id: "medium-ad", slot: "7002636040", format: "auto" },      // medium ads
    { id: "bottom-ad", slot: "8118909613", format: "autorelaxed" } // bottom (multiple) ads
  ];

  manualAds.forEach(ad => {
    const container = document.getElementById(ad.id);
    if (container) {
      const isMultiplex = ad.format === 'autorelaxed';
      // The 'data-full-width-responsive' attribute is for display ads, not multiplex ads.
      const responsiveAttr = !isMultiplex ? 'data-full-width-responsive="true"' : '';
      ...
      try {
        ...
        console.warn(`AdSense load failed for slot ${ad.slot}:`, e);
      }
    } else {
        // This warning helps in debugging if an ad placeholder is missing in the HTML.
        console.warn(`Ad container with id '${ad.id}' not found in the document.`);
    }
  });
});
