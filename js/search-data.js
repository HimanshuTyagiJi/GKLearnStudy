...
function initializePostRendering(){
    ...
    const createPostCard=(post, index, options = {})=>{
        ...
        card.innerHTML=`<div class="card-thumbnail"><a href="/categories.html?category=${encodeURIComponent(post.category)}" class="category-badge">${post.category}</a>${readingTimeLabel}<a href="${post.url}" class="card-image-link" tabindex="-1" aria-label="${post.title.replace(/"/g, '&quot;')}">${imageHtml}</a></div><div class="card-content"><h3 class="card-title"><a href="${post.url}">${post.title}</a></h3>${summaryContent}</div>${metaBlock}`;
        return card
    };
