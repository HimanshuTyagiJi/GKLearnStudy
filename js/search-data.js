...
window.GKApp.searchData = []; // Initialize as empty array, will be populated by fetch
...
/**
 * Main application logic that depends on the search data.
 * This function is called after the data is fetched successfully.
 * @param {Array} allPosts - The array of post data from search-data.json.
 */
function initializeGkAppWithData(allPosts) {
    // --- CONFIGURATION ---
...
document.addEventListener("DOMContentLoaded", () => {
    fetch('/search-data.json')
        ...
        .then(data => {
            // Make data globally available for the search bar in index.js and other scripts
            window.GKApp.searchData = data;
            // Initialize all data-dependent parts of the application
            initializeGkAppWithData(data);
        })
        .catch(error => {
